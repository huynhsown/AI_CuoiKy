...
pygame.init()
DROP_SOUND = pygame.mixer.Sound("DropItem.mp3")
WIN_SOUND = pygame.mixer.Sound("Win.mp3")
CLICK_OPTION_SOUND = pygame.mixer.Sound("ClickOption.mp3")
OPTION_SIZE = 45
players = [Human_Player(), AI_Player(Negamax(5))]
    ...
    def draw_grid(self, board):
        ...
        for row in range(ROWS):
            for col in range(COLS):
                ...
                button = CircleButton((x,y), "", self.get_font(1), WHITE, color, CIRCLE_RADIUS)
        pygame.display.update()
...
class Game:
    def __init__(self):
        self.controller = GameController(players)
        self.view = GameView()
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if not self.controller.is_over():
                        pos = check_in_circle(pygame.mouse.get_pos())
                        if pos is None:
                            continue
